...
class JarvisControl:
    ...
    def abrir_aplicativo(self, nome_app):
        ...
        try:
            apps = {
                "spotify": "spotify:",
                "valorant": "riotclient://launch-product=valorant&patchline=live",
                "riot client": "riotclient://launch-product=riot_client&patchline=live",
            ...
            comando = apps.get(nome_limpo)

            if comando:
                # Se for um protocolo (contém : e não começa com start)
                if ":" in comando and not comando.startswith("start "):
                    os.startfile(comando)
                    return f"Iniciando {nome_app}."
                
                # Se o comando for para abrir um executável
                executavel = comando.replace("start ", "", 1).strip()
                try:
                    os.startfile(executavel)
                except:
                    # Fallback usando o shell cmd 'start' para maior robustez (Word, Excel, etc)
                    subprocess.Popen(['cmd', '/c', 'start', '', executavel], shell=True)
                return f"Abrindo {nome_app}."
            
            # Caso não esteja no mapeamento, tenta o melhor esforço
            try:
                if ":" in nome_app: os.startfile(nome_app)
                else: os.startfile(nome_app + (".exe" if not nome_app.endswith(".exe") else ""))
            except:
