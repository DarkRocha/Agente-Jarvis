...
async def _abrir_chrome_com_cdp(url: str = "about:blank"):
    """
    Tenta abrir a URL usando Playwright (CDP) se o Chrome já estiver com a porta aberta.
    Caso contrário, abre normalmente via navegador sem fechar as guias existentes.
    """
    if not CHROME_PATH:
        webbrowser.open(url)
        return False

    # Se o Chrome já está aberto com CDP, usa Playwright para abrir nova aba
    if _cdp_disponivel():
        try:
    ...
    # Fallback: Abre no navegador padrão/Chrome sem matar o processo
    webbrowser.open(url)
    return True
...
async def entrypoint(ctx: agents.JobContext):
    ...
    try:
        ...
        # get_all() recupera TODAS as memórias do usuário
        results = await mem0_client.get_all(user_id=user_id)
        # O retorno do get_all é diretamente uma lista
    ...
    async def shutdown_hook():
        try:
            msgs = []
            # Usamos 'agent' diretamente para garantir o acesso ao contexto da instância
